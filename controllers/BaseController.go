...
func (c *BaseController) checkLogin(ignores ...string) {
	...
	if c.curUser.Id == 0 {
		...
		if c.Ctx.Input.IsAjax() {
			...
			c.jsonResult(utils.JRCode302, "请登录", urlstr+returnURL)
		}
...
func (c *BaseController) checkAuthor(ignores ...string) {
	...
	if !hasAuthor {
		...
		if !hasAuthor {
			if c.Ctx.Input.IsAjax() {
				c.jsonResult(utils.JRCode401, "无权访问", "")
			} else {
				c.pageError("无权访问")
			}
		}
...
//app.conf里面读取系统信息
func (c *BaseController) setAppInfo() {
	...
	c.Data["giturl"] = beego.AppConfig.String("giturl")
}
